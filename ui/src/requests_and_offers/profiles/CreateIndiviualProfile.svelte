...
async function createIndiviualProfile() {  
  const indiviualProfileEntry: IndiviualProfile = { 
    name: name!,
    nickname: nickname!,
    bio: bio!,
  };
  ...
  try {
    const record: Record = await client.callZome({
      ...
      payload: indiviualProfileEntry,
    });
    dispatch('indiviual-profile-created', { indiviualProfileHash: record.signed_action.hashed.hash });
  } catch (e) {
...
<div style="display: flex; flex-direction: column">
  <span style="font-size: 18px">Create IndiviualProfile</span>
  ...
  <mwc-button 
    raised
    label="Create IndiviualProfile"
    disabled={!isIndiviualProfileValid}
    on:click={() => createIndiviualProfile()}
  ></mwc-button>
</div>
