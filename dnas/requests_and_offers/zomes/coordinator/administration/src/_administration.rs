...
#[hdk_extern]
fn check_if_person_is_administrator(
    person_profile_hash: ActionHash,
) -> ExternResult<bool> {
    let links = get_all_administrators_links(())?;
    if links.iter().any(|link| link.target == person_profile_hash.clone().into()) {
        return Ok(true);
    }
...
fn check_if_agent_is_administrator(agent_pubkey: AgentPubKey) -> ExternResult<bool> {
    ...
    let links = get_all_administrators_links(())?;
    if links.iter().any(|link| link.target == my_profile_hash.clone().into()) {
        return Ok(true);
    }
    Err(wasm_error("Not an administrator".into()))
}
